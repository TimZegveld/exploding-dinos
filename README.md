...
Rooms gebruiken lokaal standaard tijdelijke geheugenopslag. Productie gebruikt `ROOM_STORE=redis` en vereist een geheime `REDIS_URL` naar een Redis-/Valkey-compatibele store. Iedere room wordt als één versieerbaar document met een TTL van twaalf uur opgeslagen; spelacties gebruiken atomische optimistic locking, zodat twee verzoeken op dezelfde roomversie niet allebei worden toegepast. De server weigert in Redis-modus te starten wanneer `REDIS_URL` ontbreekt en valt in productie dus niet ongemerkt terug op geheugenopslag. Koppel in Render een Key Value-instance in dezelfde regio via de interne URL. Een gratis Key Value-instance overleeft herstarts van de webservice, maar Render biedt disk-backed persistentie alleen op betaalde Key Value-instances; kies daarvoor `Journal + Snapshot` wanneer rooms ook een herstart van de datastore zelf moeten overleven.
...
- Multiplayer gebruikt deze lokale aftelling niet. Zodra een online spel wordt weergegeven, worden achtergebleven singleplayertimers en reveal-state geannuleerd; online bevestigingen blijven volledig servergestuurd en handmatig.
- Bij de eerste weergave van een online potje toont de dino-race de servergekozen startspeler en de volledige speelvolgorde. Iedere speler bevestigt deze melding zelf; er is geen multiplayeraftelling.
- Online winst en verlies gebruiken een compacte verticale eindkaart met uitslag, illustratie en vervolgstap. Op mobiel verdwijnen het losse actie- en uitlegpaneel; dezelfde functies blijven in het hamburgermenu beschikbaar.
